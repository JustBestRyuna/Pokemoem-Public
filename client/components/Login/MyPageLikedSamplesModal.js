...
const MyPageSamplesModal = (props) => {
    ...
    const checkIfLiked = async () => {
        try {
            setError(null);
            setLoading(true);
            const response = await httpGet.get(`/pokedex/checkifliked/${props.sampleData.sample_id}/${props.authenticated}/${props.userid}`);
            if (response.data) {
                setLiked(true);
            }
        } catch (e) {
    ...
    useEffect(() => {
        ...
        fetchNature();
        checkIfLiked();
        const serverDate = new Date(parseInt(props.sampleData.created));
    ...
    const handleLike = async () => {
        try {
            setError(null);
            setLoading(true);
            await httpGet.get(`/pokedex/likesample/${props.sampleData.sample_id}/${props.authenticated}/${props.userid}`);
            setLiked(true);
            props.onlikechange();
        } catch (e) {
            setError(e);
    ...
    const handleUnlike = async () => {
        try {
            setError(null);
            setLoading(true);
            await httpGet.get(`/pokedex/unlikesample/${props.sampleData.sample_id}/${props.authenticated}/${props.userid}`);
            setLiked(false);
            props.onlikechange();
        } catch (e) {
            setError(e);
    ...
    const handleDelete = async () => {
        try {
            ...
            if (deleteConfirm) {
                await httpPost.delete(`/pokedex/deletesample/${props.sampleData.sample_id}`);
    ...
    return (
        <PokedexSamplesModalStyle>
            ...
            <h1>{props.sampleData.name_ko}: {props.sampleData.title}</h1>
            ...
            <h2>샘플 상세</h2>
            <SampleSpan>
                {props.sampleData.name_ko} @ {props.sampleData.item}<br />
                ...
                <StatsSpan>(레벨 50 기준 실수치: 
                    {parseInt(((props.sampleData.hp * 2) + props.sampleData.ivh + parseInt(props.sampleData.evh / 4) + 100) * 0.5) + 10}- 
                    {parseInt((parseInt(((props.sampleData.attack * 2) + props.sampleData.iva + parseInt(props.sampleData.eva / 4)) * 0.5) + 5) * nature.a)}- 
                    {parseInt((parseInt(((props.sampleData.defense * 2) + props.sampleData.ivb + parseInt(props.sampleData.evb / 4)) * 0.5) + 5) * nature.b)}- 
                    {parseInt((parseInt(((props.sampleData.spatk * 2) + props.sampleData.ivc + parseInt(props.sampleData.evc / 4)) * 0.5) + 5) * nature.c)}- 
                    {parseInt((parseInt(((props.sampleData.spdef * 2) + props.sampleData.ivd + parseInt(props.sampleData.evd / 4)) * 0.5) + 5) * nature.d)}- 
                    {parseInt((parseInt(((props.sampleData.speed * 2) + props.sampleData.ivs + parseInt(props.sampleData.evs / 4)) * 0.5) + 5) * nature.s)})
                </StatsSpan><br />
            ...
            <LeftAlign>
                <Button onClick={e => Router.push(`/pokedex/${props.sampleData.name_en}`)} style={{WebkitAppearance: 'none'}}>
                    <img src={`/images/${props.sampleData.menu_icon}`}/>{props.sampleData.name_ko} 바로가기</Button>
            </LeftAlign>
...
export default MyPageSamplesModal;